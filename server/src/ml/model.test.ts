...
function mk(i: number, win: number, score: number, hour: number): Sample {
  const f = computeFeatures({ bars: [{ time: Date.UTC(2026, 0, 1, hour), open: 100, high: 101, low: 99, close: 100, volume: 10 }], i: 0, ev: { side: 'long', score, source: 'alert' }, entry: 100, sl: 99, tp1: 102, atr: 1, levelsSource: 'script', leverage: 5 });
...
test('features have the declared shape', () => {
  const f = computeFeatures({ bars: [{ time: 0, open: 1, high: 1, low: 1, close: 1, volume: 1 }], i: 0, ev: { side: 'short', source: 'shape' }, entry: 1, sl: 1.01, atr: 0.01, levelsSource: 'atr-fallback', leverage: 10 });
  for (const n of FEATURE_NAMES) assert.ok(Number.isFinite(f[n]), n);
  assert.equal(f.side_long, 0); assert.equal(f.has_score, 0); assert.equal(f.src_shape, 1); assert.equal(f.leverage, 10);
});
